...
CWarpingModule::CWarpingModule( ) {
    ...
    m_pnts3d = CWarpUtils::LoadMeanPnts3D("./model/3d.txt"); // load 3D points to a cv::Mat (9*3)
...
void CWarpingModule::WarpToFrontalFace3D( const Mat& _img, const PointSetd& _pnts, bool _isWarp ) {
	...
	FOR (i, NUM_POINTS) {
		...
		tgtPnts[i] = Point2d(m_pnts3d.at<double>(i,0), m_pnts3d.at<double>(i,1)); //  model里的3d.txt, 9个landmarks，取每个点的x，y坐标
		tgtPnts3d[i] = Point3d(tgtPnts[i].x, tgtPnts[i].y, m_pnts3d.at<double>(i, 2)); // 3d.txt, 取每个点的三维坐标
	}

	// 2D alignment 
	double m[6]={0};
	// M 是 2*3 矩阵，所以还是2D的 affine matrix
